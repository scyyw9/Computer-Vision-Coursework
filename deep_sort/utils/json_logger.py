...
class BboxToJsonLogger(BaseJsonLogger):
    ...
    Attributes:
        ...
        start_time (datetime object): we use it to automate the json outputs by time.
    ...
    def json_output(self, output_name):
        ...
        Notes:
            It creates the json outputs with `output_name` name.
        """
    ...
    def schedule_output_by_time(self, output_dir=JsonMeta.PATH_TO_SAVE, hours: int = 0, minutes: int = 0,
        ...
        Args:
            output_dir (str): the directory where outputs files will be stored
            hours (int):
            minutes (int):
            seconds (int):
    ...
    def flush(self, output_dir):
        """
        Notes:
            We use this function to outputs jsons whenever possible.
