...
def detect_and_track(input_path: str, output_path: str, detect_class: int, model, tracker) -> Path:
    ...
    output_video_path = Path(output_path) / "outputs.avi"  # 设置输出视频的保存路径。
    ...
    print(f'outputs dir is: {output_video_path}')
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    output_dir = gr.Textbox(label="Output dir",
                                            value=tempfile.mkdtemp())  # 文本框控件，用于指定输出视频的保存路径，默认为一个临时生成的目录
                    with gr.Row():
